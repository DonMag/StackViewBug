...
@interface NotProblemViewController ()
@property (strong, nonatomic) UIStackView *stack;
@property (strong, nonatomic) ProblemView *myView1;
@property (strong, nonatomic) ProblemView *myView2;
@end

@implementation NotProblemViewController
...
- (void)viewDidLoad {
	...
	_myView1 = [ProblemView new];
	_myView2 = [ProblemView new];
	...
	v.numberOfLines = 0;
	v.text = @"We're not animating, so tapping Show or Hide works as expected.";
...
- (void) showTapped {
	...
	// issue only happens if animated
	_myView2.hidden = NO;
